...
class MemCache extends \BanditBB\Data\Cache\DataCache implements \BanditBB\Data\CacheStructure {

    protected static $instance;
    protected static $cache = [];
    protected static $memCacheHandle = null;

    public function __construct() {
        try {
            self::$memCacheHandle = new Memcache();
            self::$memCacheHandle->connect( self::settings()->memcacheServerHost, self::settings()->memcacheServerPort );
        } catch ( \Exception $e ) {
            throw new \BanditBB\Core\Exceptions\CacheException( \sprintf( 'Unable to connect to the configured memcache server at %s:%s', self::settings()->memcacheServerHost, self::settings()->memcacheServerPort ) );
        }
    }
    ...
    public static function build() {
        foreach ( self::$tables as $table ) {
            ...
            if ( self::$memCacheHandle->get( $table ) != null ) {
                self::$cache[$table] = \json_decode( self::$memCacheHandle->get( $table ) );
            } else {
                ...
                self::$memCacheHandle->set( $table, $toCache, 0, self::settings()->memcacheExpirationSeconds );
